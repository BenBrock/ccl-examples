...
local_tensor = torch.zeros(local_n, dtype=torch.float32, device=get_device())
dest_tensor = torch.zeros(n, dtype=torch.float32, device=get_device())

tensor_list = []

for i in range(dist.get_world_size()):
    tensor_list.append(dest_tensor[i*local_n:(i+1)*local_n])
...
for dest_rank in range(dist.get_world_size()):
    dist.barrier()
    begin = time.time()

    if dist.get_rank() == dest_rank:
        dist.gather(local_tensor, gather_list=tensor_list, dst=dest_rank)
    else:
        dist.gather(local_tensor, gather_list=None, dst=dest_rank)
    ...
    if dist.get_rank() == dest_rank:
        print('Gather -> %s took %s ms, achieved %s GB/s' % (dest_rank, 1000*duration, gbytes / duration))
